...
function generateSelectedPortfolioPDFs(sendEmails) {
  ...
  var selectedStudentEmails = getStudentsToUpdate();  
...
function keepKillPortfolioSheets() {
  console.log("START keepKillUnwantedPortfolioSheets()");
  ...
  for (var i = 0; i < students.length; i++) {
    ...
    console.warn("REAL: keepSheets: %s, killSheets:", keepPatterns, killPatterns);
    keepKillSheets(ss, keepPatterns, killPatterns);
